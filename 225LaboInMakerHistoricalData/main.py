...
class Command(object):
    ...
    def make_peroid_history_03(
        ...
        origin_df = pd.read_csv(
            i_csv_fname_1min, encoding="utf-8", parse_dates=["datetime"]
        )

        # 日付のリスト
        origin_day = origin_df["day"].unique()
        ...
        for day in origin_day:
            ...
            # 先頭の日付を開始時間にしている
            start_time = day_df["datetime"].iloc[0]

            while start_time < day_df["datetime"].iloc[-1]:
                ...
                if len(term_df) > 0:
                    ...
                    high_price = term_df["high"].max()
                    low_price = term_df["low"].min()
    ...
    def make_peroid_history_02(
        ...
        for day in origin_day:
            ...
            while b_loop:
                ...
                result_list.append(
                    [start_time, open_price, high_price, low_price, close_price, volume]
                )
    ...
    def _add_csv_data(self, wb: openpyxl.Workbook, target_sheet_names: list):
        for sheet_name in target_sheet_names:
            ...
            print(
                "sheet_name({}): first_row({}) / max_row({})".format(
                    sheet_name, first_row, last_row
                )
            )
